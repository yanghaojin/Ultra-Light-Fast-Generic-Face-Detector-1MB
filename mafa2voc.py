...
# This is a modified version to convert the MAFA dataset to the pascal voc compatible format.
...
def parse_labels(raw_labels, is_train=True):
    ...
    if is_train:
        for raw_label in raw_labels:
            # filtering
            if raw_label[12] == 3 or raw_label[13] < 3:
                continue
        ...
        for raw_label in raw_labels:
            # filtering
            if raw_label[9] == 3 or raw_label[10] < 3:
                continue
...
def write_voc_style_ann(labels, img_file_name):
...
if __name__ == "__main__":
    ...
    with open(listfile, 'w') as f:
        ...
        for sample in tqdm(samples):
            ...
            f.write(os.path.splitext(img_file_name)[0])
            f.write('\n')

            # create xml annotation file
            xml_annotation_file = path.join(export_annotations_dir, img_name_wo_ext + '.xml')
            f_xml = write_voc_style_ann(labels, img_file_name)
            f_xml.write(xml_annotation_file)

            # Extract the face images.
            if args.export_dir is not None:
                export_face_image_file = path.join(export_face_image_dir, img_file_name)
                # copy images
